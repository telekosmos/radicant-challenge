...
class FundRepositoryPgInterpreter[F[_]: Async](val xa: Transactor[F]) extends FundRepostoryAlgebra[F] {
  private object Statements {
    ...
    def getBySizeAndSector(size: Long, sector: String) = {
      val sectorField = sector2Fields(sector)
      ...
      val sectorFr = fr"and " ++ Fragment.const(sectorField) ++ fr" > greatest(" ++ restOfSectors ++ fr")"
      (sql"""
...
object FundRepositoryPgInterpreter {
  def apply[F[_]: Async](transactor: Transactor[F]): FundRepositoryPgInterpreter[F] =
    new FundRepositoryPgInterpreter[F](transactor)
}